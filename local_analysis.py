...
def main():
    ...
    root_save_analysis_path = os.path.join('./saved_visualizations',
                                           experiment_run)
    makedir(root_save_analysis_path)

    log, logclose = create_logger(log_filename=os.path.join(
        root_save_analysis_path, 'local_analysis.log'))
    wandb_logger = WandbLogger({},
                               logger_name='ProtoPNet_Test',
    ...
    prototype_info = np.load(
        os.path.join(load_img_dir, 'epoch-' + epoch_number_str,
                     'bb' + epoch_number_str + '.npy'))
    prototype_img_identity = prototype_info[:, -1]

    log('Prototypes are chosen from ' + str(len(set(prototype_img_identity))) +
        ' number of classes.')
    ...
    if np.sum(prototype_max_connection ==
              prototype_img_identity) == ppnet.num_prototypes:
    ...
    def imsave_with_bbox(fname,
                         ...
                         bbox_width_end,
                         color=(0, 255, 255)):
        img_bgr_uint8 = cv2.cvtColor(np.uint8(255 * img_rgb),
                                     cv2.COLOR_RGB2BGR)
    ...
    for test_image_name in os.listdir(test_image_dir):
        if not test_image_name.lower().endswith(
                ".jpeg") or test_image_name.lower().endswith(".jpg"):
            continue
        save_analysis_path = os.path.join(root_save_analysis_path,
                                          os.path.splitext(test_image_name)[0])
        ...
        test_image_label = test_dataset.class_to_idx[test_image_name.split("-")
                                                     [0]]
        ...
        for i in range(1, 11):
            ...
            save_prototype_original_img_with_bbox(
                ...
                bbox_height_start=prototype_info[
                    sorted_indices_act[-i].item()][1],
                bbox_height_end=prototype_info[
                    sorted_indices_act[-i].item()][2],
                bbox_width_start=prototype_info[
                    sorted_indices_act[-i].item()][3],
                bbox_width_end=prototype_info[
                    sorted_indices_act[-i].item()][4],
                color=(0, 255, 255))
        ...
        for i, c in enumerate(topk_classes.detach().cpu().numpy()):
            makedir(
                os.path.join(save_analysis_path,
            ...
            for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
                ...
                save_prototype_self_activation(
                    os.path.join(
                        save_analysis_path,
                        'top-%d_class_prototypes' % (i + 1),
                        'top-%d_activated_prototype_self_act.png' %
                        prototype_cnt), start_epoch_number, prototype_index)
                log('prototype index: {0}'.format(prototype_index))
                log('prototype class identity: {0}'.format(
                    prototype_img_identity[prototype_index]))
                if prototype_max_connection[
                        prototype_index] != prototype_img_identity[
                            prototype_index]:
                    log('prototype connection identity: {0}'.format(
                ...
                plt.imsave(
                    os.path.join(
                        ...
                        prototype_cnt), high_act_patch)
                log('most highly activated patch by this prototype shown in the original image:'
                    )
                imsave_with_bbox(fname=os.path.join(
                    save_analysis_path, 'top-%d_class_prototypes' % (i + 1),
                    'most_highly_activated_patch_in_original_img_by_top-%d_prototype.png'
                    % prototype_cnt),
                                 img_rgb=original_img,
                                 bbox_height_start=high_act_patch_indices[0],
                ...
                rescaled_activation_pattern = rescaled_activation_pattern / np.amax(
                    rescaled_activation_pattern)
                heatmap = cv2.applyColorMap(
                    np.uint8(255 * rescaled_activation_pattern),
                    cv2.COLORMAP_JET)
                heatmap = np.float32(heatmap) / 255
                heatmap = heatmap[..., ::-1]
                overlayed_img = 0.5 * original_img + 0.3 * heatmap
